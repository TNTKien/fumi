...
export default class Database {
  ...
  async get(id) {
    ...
    return (
      await this.client.db
        .prepare(`SELECT * FROM ${this.table} WHERE discordID = ?1;`)
        .bind(id)
        .all()
    ).results[0];
  }
  ...
  async update(id, obj) {
    ...
    const query = `INSERT INTO "${this.table}" ("discordID", ${keys
      .map((key) => `"${key}"`)
      .join(', ')}) VALUES (?1, ${keys
      .map((_, i) => `?${i + 2}`)
      .join(', ')}) ON CONFLICT ("discordID") DO UPDATE SET ${keys
      .map((key, i) => `"${key}" = ?${i + 2}`)
      .join(', ')} RETURNING *;`;
    const { results: rows } = await this.client.db
      .prepare(query)
      .bind(id, ...values)
      .all();
    return rows[0];
  ...
  async newUser(discordID, hsrID) {
    const query = `INSERT INTO "${this.table}" ("discordID", "hsrID") VALUES (?1, ?2);`;
    await this.client.db.prepare(query).bind(discordID, hsrID).run();
    const selectQuery = `SELECT * FROM "${this.table}" WHERE "discordID" = ?1;`;
    const { results: rows } = await this.client.db
      .prepare(selectQuery)
      .bind(discordID)
      .all();
